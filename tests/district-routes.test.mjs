...
test('routed edges retain exact endpoints with finite orthogonal segments in all expansion modes',()=>{
  for(const exploded of [false,true])for(const spacing of [1,2,3]){
    const layout=layoutGraph(graph,'district',exploded,{spacing,selectedFloor:'server:tags'}),edges=graph.edges.map(signalEdge),routes=districtRoutes(edges,layout.positions,layout.districts);
...
test('inter-floor vertical risers stay outside container shells and routes get distinct lanes',()=>{
  const layout=layoutGraph(graph,'district',true,{spacing:3}),edges=graph.edges.map(signalEdge),routes=districtRoutes(edges,layout.positions,layout.districts),front=Math.max(...layout.districts.map(f=>f.z+f.depth/2));
  const lanes=[];
...
test('maximum spread increases floor separation, entity spacing, and pull-out distance',()=>{
  const normal=layoutGraph(graph,'district'),wide=layoutGraph(graph,'district',true,{spacing:3}),selected=layoutGraph(graph,'district',true,{spacing:3,selectedFloor:'server:tags'});
...
test('self references produce a visible loop and missing nodes do not invent routes',()=>{
  const layout=layoutGraph(graph,'district'),edge={from:'tag:2',to:'tag:2',kind:'tag_setup'},routes=districtRoutes([edge,{from:'missing',to:'tag:2',kind:'tag_setup'}],layout.positions,layout.districts);
